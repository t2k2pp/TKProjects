    ...
    def convert_html_to_png(self):
        ...
        try:
            ...
            return {
                ...
                height: calculatedHeight + 50 // Adding a 50px buffer
            };
            """
            ...
            with Image.open(saved_image_path) as img:
                ...
                if not size_specified: 
                    ...
                    # If the original image was RGBA and user wants transparency, ensure resized_content is RGBA.
                    # If the original image was RGB or user wants a specific background color,
                    # ensure resized_content is composited onto that color.
                    if self.transparent_var.get():
                        if resized_content.mode != 'RGBA':
                            processed_img = resized_content.convert('RGBA')
                        else:
                            processed_img = resized_content
                    else:
                        # Not transparent, composite onto background color
                        bg_hex = self.bg_color_var.get()
                        try: 
                            bg_rgb_canvas = tuple(int(bg_hex[i:i+2], 16) for i in (1,3,5))
                        except: 
                            bg_rgb_canvas = (255,255,255) # Default to white if color is invalid
                        
                        # Create a new image with the determined background color and dimensions new_w, new_h
                        final_image_bg = Image.new('RGB', (new_w, new_h), bg_rgb_canvas)
                        
                        # If resized_content has alpha, composite it. Otherwise, just ensure it's RGB.
                        if resized_content.mode == 'RGBA':
                            # Paste using alpha channel as mask for proper blending
                            final_image_bg.paste(resized_content, (0,0), resized_content) 
                            processed_img = final_image_bg
                        else:
                            # If no alpha, convert to RGB and paste (though direct assignment might work if modes match)
                            # For safety, create the background and paste.
                            final_image_bg.paste(resized_content.convert('RGB'), (0,0))
                            processed_img = final_image_bg
                    
                    processed_img.save(saved_image_path, 'PNG')
                    final_output_width, final_output_height = processed_img.size 
